 ...
 * 
 * 3. 질의 : Statement객체 또는 PreparedStatement객체를 이용하여 SQL 문장을 실행한다.
 * 
 * 4. 결과 :
 ...
 */
	...
	public static void main(String[] args) {
		...
		try {
			...
			//SQL문이 select일 경우에는 executeQuery()메서드를 사용하고, insert, update, delete일 경우에는 executeUpdate()메서드를 사용한다.
			
			//5. ResultSet객체에 저장되어 있는 자료를 반복문과 next()메서드를 이용하여 차례대로 읽어와서 처리한다.
			System.out.println("실행한 쿼리문 : " + sql);
			System.out.println("=== 쿼리문 실행 결과 ===");
			
			//rs.next() => ResultSet 객체의 데이터를 가리키는 포인터를 다음 레코드로 이동시키고 그 곳에 자료가 있으면 true, 없으면 false를 반환한다.
			while(rs.next()) {
				//컬럼의 자료를 가져오는 방법
				//방법1) rs.get자료형이름("컬럼명")
...
//				System.out.println("lprod_id : " + rs.getInt("lprod_id"));
			...
			//6. 종료(사용했던 자원을 모두 반납한다.) //거꾸로 반납하는게 정석~
