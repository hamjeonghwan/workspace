 ...
 *  UnicastRemoteObject객체를 상속하여 작성한다
 * @author pc21
 *
 */
public class RemoteServer extends UnicastRemoteObject implements RemoteInterface{
   ...
   public static void main(String[] args) {
      try {
         ...
         // 포트 번호를 지정하여 Registry객체 생성(기본포트값 : 1099)
         Registry reg = LocateRegistry.createRegistry(8888);
         
         // Registry서버에 제공하는 객체 등록
         // 형식) Registry객체변수.rebind("객체의Alias", 등록할 객체변수);
         reg.rebind("server",inf); // rebind는 이미등록되있어도 또한번등록
   ...
   public void setFiles(FileInfoVO[] fInfo) throws RemoteException{
      ...
      for(int i = 0; i<fInfo.length; i++) {
         try {
            ...
            //클라이언트에서 전달한 파일데이터 (bytep[])를 서버측에 전달한다.
